...
class LLMClient:
    ...
    async def translate(
        ...
        if direction == "tab-rus":
            source_lang = "табасаранского"
            target_lang = "русский"
        else:
            source_lang = "русского"
            target_lang = "табасаранский"

        # Format context from dictionary
        context_text = self._format_context(context)

        prompt = f"""Ты - переводчик с {source_lang} языка на {target_lang}.

Используй следующий контекст из словаря для перевода:
{context_text}

Переведи следующий текст, сохраняя смысл и стиль:
"{text}"
